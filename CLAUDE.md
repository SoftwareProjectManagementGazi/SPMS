# CLAUDE.md - Python/FastAPI Clean Architecture Guidelines
...
- **Frustrations:** Before modifying any working code, mentally trace what depends on it and verify the change is safe. Re-read user instructions before claiming a task is done; do not skip stated constraints. After implementation, double-check wiring (imports, prop names, event handlers, type signatures, async/await, snake_case vs camelCase) for small semantic mistakes. Treat 'job done' as 'job done correctly without breaking previous phases', not just 'code compiles'.
