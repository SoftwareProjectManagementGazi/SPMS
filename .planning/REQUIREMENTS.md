...
- [ ] **RBAC-01**: Permission domain layer kurulur — `Permission`, `RolePermission` entity'leri, `IPermissionRepository`/`IRolePermissionRepository`/`IRoleRepository` ABC'leri, SqlAlchemy impl + mapper'lar; Alembic migration 007 (permissions + role_permissions tablolari + roles.is_system_role/icon_key/color_token + permissions.scope VARCHAR+CHECK + 38 perm seed: 26 project (project.*/task.*/comment.*/milestone.*/artifact.*/phase_report.*/workflow.edit/lifecycle.edit/member.*) + 12 admin.* + role_permissions matrix bootstrap (PM 23 / Member 5 / Admin 0 / Guest 0), idempotent) [Phase 15]
