...
- [x] **Wave 4 — Gemini Integration** (2026-05-20) — Real Gemini 2.5 Flash adapter via google-genai SDK. Pydantic response_schema enforcement (Issue #699 workaround: default-free schema variant). Two prompt builders (lifecycle, task-status) with methodology hints, sector context, deployment hints, quality flag mappings, prompt injection defense (`<user_context>` tag), and few-shot examples. Auto-layout fallback (BFS-based) for cases where Gemini omits x/y. Error mapping (429 → rate_limit, 5xx → service_down). CRITICAL FIX: `thinking_budget=0` — Gemini 2.5 Flash thinking model ate all max_output_tokens leaving partial JSON; disabling thinking + raising max_output_tokens to 8000 resolved truncation. End-to-end verified with ITERATIVE/SCRUM/etc. methodologies.
- [x] **Wave 5 — Apply & Production Polish** (2026-05-20) — Apply confirmation overlay (D-01 default "replace", D-02 always-show), apply service mapping AI suggestion → WorkflowConfig with node-id normalization (backend D-22 regex) and isInitial/isFinal failsafes (rules 2+4), backend PATCH/POST dispatch, new_project required-field synthesis (key, start_date, methodology), success toast + page reload to sync local editor state. 3-tier rate limiter (D-05: user-hour=8, user-day=25, project-day=400) wired inline into both AI endpoints. Cold-start UX: early text_token before Gemini call + 60s fetch timeout. AIErrorState component with calm centered card for service_down + rate_limit (countdown), "Şablonlara Git" + "Tekrar Dene" fallback affordances. CRITICAL FIX: lifecycle/task-status prompt clarity — Scrum was producing sprint ceremonies as project phases; system prompts now explicitly distinguish "PROJE YAŞAM DÖNGÜSÜ" from sprint events and task statuses.
- [x] **Wave 6 — Polish & Smoke Test (partial)** (2026-05-20) — Wave 3 deferred animation glitch RESOLVED: ai-node-appear keyframe was using `transform: scale()` which conflicted with React Flow's per-node `transform: translate(x, y)` applied to the same wrapper; nodes briefly appeared off-position before snapping. Switched to opacity-only fade (220ms ease-out) — animation feel preserved, no transform conflict. 7-methodology Gemini smoke test PASSED: SCRUM (6/7), KANBAN (7/7), WATERFALL (8/8), ITERATIVE (8/8), INCREMENTAL (6/7), EVOLUTIONARY (6/7), RAD (7/8) — all 7 pass domain validator, all produce Türkçe phase names + rationale.
